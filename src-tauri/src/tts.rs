...
/// Placeholder engine used while the real `SherpaEngine` is being built on
/// a background thread, AND for the terminal "load failed" state where we
/// don't want to keep a half-built SherpaEngine around. Carries a `detail`
/// string so the UI can distinguish "loading" from the various failure
/// modes (no models / corrupt files / etc.) without us having to invent a
/// second placeholder type.
